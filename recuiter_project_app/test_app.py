...
def main():
    ...
    with st.sidebar:
        ...
        with st.form("config_form"):
            ...
            # Get current job cities from session state
            current_job_cities = st.session_state.get('job_cities', [])

            # Combine base cities with any custom cities from session state
            ...
            st.subheader("إعدادات الموارد البشرية")
            hr_email = st.text_input("بريد الموارد البشرية", value=os.getenv("HR_FROM_EMAIL", ""))
            form_link = st.text_input("رابط Google Form", value=os.getenv("FORM_Link", ""))
            if form_link:
                st.markdown(f"[فتح النموذج]({form_link})", unsafe_allow_html=True)
            ...
            evaluation_mode = st.selectbox(
                ...
                ["تقييم السيرة الذاتية فقط", "تقييم السيرة الذاتية + الاختبار"],
            ...
            if st.form_submit_button("🚀تشغيل البرنامج"):
                ...
                # Store cities in session state
                st.session_state.job_cities = job_cities
                
                # FIX: Save to environment and session state
                ...
                # FIX: Set environment variables immediately
